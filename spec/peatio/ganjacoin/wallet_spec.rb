RSpec.describe Peatio::Ganjacoin::Wallet do
  let(:wallet) { Peatio::Ganjacoin::Wallet.new }

  let(:uri) { 'http://user:password@127.0.0.1:12310' }
  let(:uri_without_authority) { 'http://127.0.0.1:12310' }
  ...
  context :configure do
    let(:unconfigured_wallet) { Peatio::Ganjacoin::Wallet.new }
    ...
    it 'sets settings attribute' do
      ...
      expect(unconfigured_wallet.settings).to eq(settings.slice(*Peatio::Ganjacoin::Wallet::SUPPORTED_SETTINGS))
    end
  end
  ...
  context :create_address! do
    ...
    it 'request rpc and creates new address' do
      ...
      expect(result.symbolize_keys).to eq(address: 'mrjatest:qqpkpjpsv5ngwjet9alaktdhklgcrlf9qg9l4ta2w5')
    end
  end
  ...
  context :create_transaction! do
    ...
    let(:transaction) do
      Peatio::Transaction.new(amount: 0.11, to_address: 'mrjatest:qqpkpjpsv5ngwjet9alaktdhklgcrlf9qg9l4ta2w5')
    end
    ...
    it 'requests rpc and sends transaction without subtract fees' do
      ...
      expect(result.to_address).to eq('mrjatest:qqpkpjpsv5ngwjet9alaktdhklgcrlf9qg9l4ta2w5')
